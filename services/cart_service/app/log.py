"""Configuration of Loguru for cart_service."""
...
def configure_logging():
    ...
    logger.add(
        os.path.join(settings.loguru.logs_dir, settings.loguru.cart_log_name),
        level=settings.loguru.log_level,
        filter=lambda record: record["extra"].get("route_group") == "cart",
        **common,
    )
    logger.add(
